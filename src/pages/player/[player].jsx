...
const Player = ({ iframeUrl,title,image,MapedData,download,player}) => {
  ...
  return (
    <>
      ...
      <div style={{display:"flex",justifyContent:"center",width:"100vw",margin:"50px 0"}}><div style={{display:"flex",alignItems:"center"}}><span style={{color:"red"}}>Info :- </span><p> This website is for sale, Please contact for best price :- <Link href="https://www.instagram.com/vijayji1137/" style={{color:"orange"}}>Instagram</Link></p></div></div>
      <Link href="https://beta.publishers.adsterra.com/referral/aKMU588PJU">
        <Image
          alt="banner"
      ...
      <div className='grid grid-cols-5 gap-3 mb-16'>
        {MapedData.map((item)=>{
            return <div key={item.ID} className='aspect-video'>
                <Link href={`/player/${item.ID}`} target='_blank' onClick={()=>{setTimeout(() => {router.push("https://playgroundordinarilymess.com/vzu6z5kf?key=0e1c984fe1a496834799af2ac36250d7")}, 500);}}><Image className='bg-stone-400 rounded h-full w-full cursor-pointer transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-125 hover:bg-indigo-500 duration-300' src={item.Image} alt={item.Title} title={item.Title} width={500} height={500} /></Link>
            </div>
        })}
      </div>
